...
fn test_edgeworth_guardrail_below_15_samples() {
    ...
    assert!(
        tuner.compute_skew_kurt().is_none(),
        "Must return None below N=15 threshold"
    );
...
fn test_edgeworth_reference_vector_precision() {
    ...
    assert!(
        (mean - 16.2).abs() < 1e-6,
        "Mean mismatch: expected 16.2, got {}",
        mean
    );
    assert!(
        (m2 - 113.62666667).abs() < 1e-6,
        "m2 mismatch: expected 113.62666667, got {}",
        m2
    );
    assert!(m3 > 0.0);
    assert!(m4 > 0.0);
    ...
    let (g1, g2) = tuner
        .compute_skew_kurt()
        .expect("Must compute skew/kurt at N=15");
    assert!(
        (g1 - 2.21894643).abs() < 1e-6,
        "Skewness g1 mismatch: expected 2.21894643, got {}",
        g1
    );
    assert!(
        (g2 - 4.00433476).abs() < 1e-6,
        "Kurtosis g2 mismatch: expected 4.00433476, got {}",
        g2
    );
    ...
    assert!(
        (adapted - expected_backoff).abs() < 1e-6,
        "Adapted backoff mismatch: expected {}, got {}",
        expected_backoff,
        adapted
    );
}
...
fn test_edgeworth_zero_variance_guardrail() {
    ...
    assert!(
        tuner.compute_skew_kurt().is_none(),
        "Must return None when variance m2 -> 0 to avoid NaN/Inf"
    );
    assert_eq!(
        tuner.adapt(),
        Some(100.0),
        "Must fall back to base backoff when variance is zero"
    );
}
